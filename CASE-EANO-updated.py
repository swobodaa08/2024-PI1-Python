...
if priezvisko:
    while pokusy > 0:
        heslo = simpledialog.askstring("Prihlásenie", "Zadajte heslo od Wi-Fi:", show="*")  # Skryté heslo
        if heslo == spravne_heslo:
            # Načítanie údajov
            konto = load_progress(priezvisko)
            casino = load_casino()

            # Zničenie hlavného okna prihlasovania
            root.destroy()
            ...
            # Zobrazenie zostatku a tlačidiel
            casino_profit = math.ceil(casino)

            profit = tk.Button(okno, text=f"Casino profit : {casino_profit}€", width=20, state="disabled", bg="pink",
                               font=('Helvetica', 25, 'bold'))
            profit.place(relx=0.5, rely=0.08, anchor='center')

            b1 = tk.Button(okno, text=f"Peňazí na účte : {konto}€", width=20, command=my_upd, bg="lime",
            ...
            update_profit()
            okno.mainloop()
            break  # Opustenie cyklu po úspešnom prihlásení

        else:
            pokusy -= 1
            if pokusy > 0:
                messagebox.showwarning("Chyba", f"Nesprávne heslo! Zostáva {pokusy} pokusov.")
            else:
                messagebox.showerror("Chyba", "Maximálny počet pokusov bol prekročený. Program sa ukončí.")
                exit()
else:
    messagebox.showerror("Chyba", "Nebolo zadané priezvisko. Program sa ukončí.")