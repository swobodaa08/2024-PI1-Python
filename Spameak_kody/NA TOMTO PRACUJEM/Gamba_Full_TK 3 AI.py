...
def pokracovat():
    ...
    for _ in range(počet_kôl):
        ...
        if tip == cislo:
            konto += suma * 3
            canvas.create_text(900, 500, text="Vyhral/-a si !!!!", font=("Helvetica", "20", "bold"), fill="white")
            canvas.create_text(900, 600, text=f"Výherné číslo bolo {cislo}, perfektne", font=("Helvetica", "20", "bold"), fill="white")
            canvas.create_text(900, 700, text=f"Počet zostávajúcich pokusov: {počet_kôl}", font=("Helvetica", "20", "bold"), fill="white")
            if konto > 1000:
                canvas.create_text(900, 800, text=f"Tvoj profit je zatiaľ {konto}€, ak budeš takto pokračovať zachvíľu z teba bude milionár :)", font=("Helvetica", "20", "bold"), fill="white")
            else:
                canvas.create_text(900, 800, text=f"Máš {konto}€ na účte, ak chceš stále hrať, pokračuj", font=("Helvetica", "20", "bold"), fill="white")
        
        else:
            canvas.create_text(900, 500, text="Prehral/-a si....", font=("Helvetica", "20", "bold"), fill="white")
            canvas.create_text(900, 600, text=f"Výherné číslo bolo {cislo}", font=("Helvetica", "20", "bold"), fill="white")
            canvas.create_text(900, 700, text=f"Počet zostávajúcich pokusov: {počet_kôl}", font=("Helvetica", "20", "bold"), fill="white")
            if konto > 500:
                canvas.create_text(900, 800, text=f"Avšak nezúfaj, zatial máš stále {konto}€", font=("Helvetica", "20", "bold"), fill="white")
            else:
                canvas.create_text(900, 800, text=f"Už máš iba {konto}€ na účte, ak chceš skončiť, vypni tento program.", font=("Helvetica", "20", "bold"), fill="white")
